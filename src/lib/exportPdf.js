...
export async function exportSessionPdf(drillCardRefs, drills, sessionName, sessionMeta = {}) {
  ...
  pdf.text(`${drills.length} harjoitetta  ·  ${totalMin} min`, MARGIN + CONTENT_W - 4, y + 10.5, { align: 'right' })
  y += headerH + 6
  ...
  if (topFields.length > 0 || focusFields.length > 0) {
    ...
    // Erotinviiva ennen harjoitteita
  ...
  const filename = (sessionName || 'harjoitus').replace(/[^a-zA-Z0-9äöåÄÖÅ _-]/g, '').trim() || 'harjoitus'
